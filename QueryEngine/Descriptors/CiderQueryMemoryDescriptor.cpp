...
#include "CiderQueryMemoryDescriptor.h"
...
std::unique_ptr<CiderQueryMemoryDescriptor> CiderQueryMemoryDescriptor::init(
    const Executor* executor,
  ...
  const auto min_slot_size = CiderQueryMemoryDescriptor::pick_target_compact_width(
  ...
  if (!is_group_by) {
    ...
    return std::make_unique<CiderQueryMemoryDescriptor>(
        executor,
        ra_exe_unit,
        query_infos,
        allow_multifrag,
  ...
  switch (col_range_info.hash_type_) {
    case QueryDescriptionType::GroupByPerfectHash: {
      ...
      keyless_hash =
          (!sort_on_gpu_hint ||
           !CiderQueryMemoryDescriptor::many_entries(
      ...
      if (group_col_widths.size() > 1) {
        ...
        interleaved_bins_on_gpu = keyless_hash && !has_varlen_sample_agg &&
                                  ...
                                  CiderQueryMemoryDescriptor::countDescriptorsLogicallyEmpty(
                                      count_distinct_descriptors) &&
                                  !output_columnar;
      }
      break;
...
CiderQueryMemoryDescriptor::CiderQueryMemoryDescriptor(
    const Executor* executor,
    const RelAlgExecutionUnit& ra_exe_unit,
    const std::vector<InputTableInfo>& query_infos,
    const bool allow_multifrag,
...
CiderQueryMemoryDescriptor::CiderQueryMemoryDescriptor(const Executor* executor,
                                             const size_t entry_count,
...
CiderQueryMemoryDescriptor::CiderQueryMemoryDescriptor(const QueryDescriptionType query_desc_type,
...
bool CiderQueryMemoryDescriptor::operator==(const CiderQueryMemoryDescriptor& other) const {
...
std::unique_ptr<QueryExecutionContext> CiderQueryMemoryDescriptor::getQueryExecutionContext(
    const RelAlgExecutionUnit& ra_exe_unit,
    const Executor* executor,
    const ExecutorDeviceType device_type,
...
int8_t CiderQueryMemoryDescriptor::pick_target_compact_width(
    const RelAlgExecutionUnit& ra_exe_unit,
    const std::vector<InputTableInfo>& query_infos,
    const int8_t crt_min_byte_width) {
  if (g_bigint_count) {
...
size_t CiderQueryMemoryDescriptor::getColsSize() const {
  return col_slot_context_.getAllSlotsAlignedPaddedSize();
}

size_t CiderQueryMemoryDescriptor::getRowSize() const {
...
size_t CiderQueryMemoryDescriptor::getWarpCount() const {
  return (interleaved_bins_on_gpu_ ? executor_->warpSize() : 1);
}

size_t CiderQueryMemoryDescriptor::getCompactByteWidth() const {
 ...
 *
 */
size_t CiderQueryMemoryDescriptor::getTotalBytesOfColumnarBuffers() const {
  CHECK(output_columnar_);
 ...
 */
size_t CiderQueryMemoryDescriptor::getTotalBytesOfColumnarBuffers(
 ...
 * no holes in it)
 */
size_t CiderQueryMemoryDescriptor::getTotalBytesOfColumnarProjections(
...
size_t CiderQueryMemoryDescriptor::getColOnlyOffInBytes(const size_t col_idx) const {
 ...
 */
size_t CiderQueryMemoryDescriptor::getColOffInBytes(const size_t col_idx) const {
 ...
 */
size_t CiderQueryMemoryDescriptor::getPrependedGroupColOffInBytes(
 ...
 * buffer.
 */
size_t CiderQueryMemoryDescriptor::getPrependedGroupBufferSizeInBytes() const {
  CHECK(output_columnar_);
...
size_t CiderQueryMemoryDescriptor::getColOffInBytesInNextBin(const size_t col_idx) const {
...
size_t CiderQueryMemoryDescriptor::getNextColOffInBytes(const int8_t* col_ptr,
                                                   const size_t bin,
...
size_t CiderQueryMemoryDescriptor::getNextColOffInBytesRowOnly(const int8_t* col_ptr,
                                                          const size_t col_idx) const {
  const auto chosen_bytes = getPaddedSlotWidthBytes(col_idx);
  const auto total_slot_count = getSlotCount();
  if (col_idx + 1 == total_slot_count) {
...
size_t CiderQueryMemoryDescriptor::getBufferSizeBytes(
    const RelAlgExecutionUnit& ra_exe_unit,
 ...
 */
size_t CiderQueryMemoryDescriptor::getBufferSizeBytes(const ExecutorDeviceType device_type,
...
void CiderQueryMemoryDescriptor::setOutputColumnar(const bool val) {
 ...
 */
bool CiderQueryMemoryDescriptor::isLogicalSizedColumnsAllowed() const {
...
size_t CiderQueryMemoryDescriptor::getBufferColSlotCount() const {
...
bool CiderQueryMemoryDescriptor::usesGetGroupValueFast() const {
  return (query_desc_type_ == QueryDescriptionType::GroupByPerfectHash &&
          getGroupbyColCount() == 1);
}

bool CiderQueryMemoryDescriptor::threadsShareMemory() const {
  return query_desc_type_ != QueryDescriptionType::NonGroupedAggregate;
}

bool CiderQueryMemoryDescriptor::blocksShareMemory() const {
...
bool CiderQueryMemoryDescriptor::lazyInitGroups(const ExecutorDeviceType device_type) const {
  return device_type == ExecutorDeviceType::GPU && !render_output_ &&
         countDescriptorsLogicallyEmpty(count_distinct_descriptors_);
}

bool CiderQueryMemoryDescriptor::interleavedBins(const ExecutorDeviceType device_type) const {
...
bool CiderQueryMemoryDescriptor::isWarpSyncRequired(
    const ExecutorDeviceType device_type) const {
...
size_t CiderQueryMemoryDescriptor::getColCount() const {
  return col_slot_context_.getColCount();
}

size_t CiderQueryMemoryDescriptor::getSlotCount() const {
  return col_slot_context_.getSlotCount();
}

const int8_t CiderQueryMemoryDescriptor::getPaddedSlotWidthBytes(const size_t slot_idx) const {
  return col_slot_context_.getSlotInfo(slot_idx).padded_size;
}

const int8_t CiderQueryMemoryDescriptor::getLogicalSlotWidthBytes(
    const size_t slot_idx) const {
  return col_slot_context_.getSlotInfo(slot_idx).logical_size;
}

const int8_t CiderQueryMemoryDescriptor::getSlotIndexForSingleSlotCol(
    const size_t col_idx) const {
...
std::string CiderQueryMemoryDescriptor::toString() const {
...
std::string CiderQueryMemoryDescriptor::reductionKey() const {
  std::string str;
...
std::vector<TargetInfo> target_exprs_to_infos(
    ...
    const CiderQueryMemoryDescriptor& query_mem_desc) {
