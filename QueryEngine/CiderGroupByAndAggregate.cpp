...
#include "CiderGroupByAndAggregate.h"
...
ColRangeInfo CiderGroupByAndAggregate::getColRangeInfo() {
...
int64_t CiderGroupByAndAggregate::getBucketedCardinality(const ColRangeInfo& col_range_info) {
...
CiderGroupByAndAggregate::CiderGroupByAndAggregate(
    Executor* executor,
...
int64_t CiderGroupByAndAggregate::getShardedTopBucket(const ColRangeInfo& col_range_info,
...
std::unique_ptr<QueryMemoryDescriptor> CiderGroupByAndAggregate::initQueryMemoryDescriptor(
...
std::unique_ptr<QueryMemoryDescriptor> CiderGroupByAndAggregate::initQueryMemoryDescriptorImpl(
    const bool allow_multifrag,
    const size_t max_groups_buffer_entry_count,
    const int8_t crt_min_byte_width,
  ...
  const auto count_distinct_descriptors = init_count_distinct_descriptors(
      ra_exe_unit_, query_infos_, device_type_);
  ...
  const auto keyless_info =
      ...
      ? KeylessInfo{false, -1}
      : get_keyless_info(ra_exe_unit_, query_infos_, is_group_by);
...
void CiderGroupByAndAggregate::addTransientStringLiterals() {
...
bool CiderGroupByAndAggregate::gpuCanHandleOrderEntries(
...
bool CiderGroupByAndAggregate::codegen(llvm::Value* filter_result,
...
llvm::Value* CiderGroupByAndAggregate::codegenOutputSlot(
...
std::tuple<llvm::Value*, llvm::Value*> CiderGroupByAndAggregate::codegenGroupBy(
    const QueryMemoryDescriptor& query_mem_desc,
    const CompilationOptions& co,
    DiamondCodegen& diamond_codegen) {
  AUTOMATIC_IR_METADATA(executor_->cgen_state_.get());
...
CiderGroupByAndAggregate::codegenSingleColumnPerfectHash(
    const QueryMemoryDescriptor& query_mem_desc,
    const CompilationOptions& co,
    llvm::Value* groups_buffer,
...
std::tuple<llvm::Value*, llvm::Value*> CiderGroupByAndAggregate::codegenMultiColumnPerfectHash(
    llvm::Value* groups_buffer,
...
std::tuple<llvm::Value*, llvm::Value*>
CiderGroupByAndAggregate::codegenMultiColumnBaselineHash(
    const CompilationOptions& co,
    llvm::Value* groups_buffer,
    llvm::Value* group_key,
    llvm::Value* key_size_lv,
...
llvm::Function* CiderGroupByAndAggregate::codegenPerfectHashFunction() {
  AUTOMATIC_IR_METADATA(executor_->cgen_state_.get());
...
llvm::Value* CiderGroupByAndAggregate::convertNullIfAny(const SQLTypeInfo& arg_type,
...
llvm::Value* CiderGroupByAndAggregate::codegenWindowRowPointer(
...
bool CiderGroupByAndAggregate::codegenAggCalls(
 ...
 */
llvm::Value* CiderGroupByAndAggregate::codegenAggColumnPtr(
...
void CiderGroupByAndAggregate::codegenEstimator(std::stack<llvm::BasicBlock*>& array_loops,
                                           DiamondCodegen& diamond_codegen,
                                           const QueryMemoryDescriptor& query_mem_desc,
                                           const CompilationOptions& co) {
  AUTOMATIC_IR_METADATA(executor_->cgen_state_.get());
...
void CiderGroupByAndAggregate::codegenCountDistinct(
    const size_t target_idx,
...
void CiderGroupByAndAggregate::codegenApproxMedian(const size_t target_idx,
                                              const Analyzer::Expr* target_expr,
                                              std::vector<llvm::Value*>& agg_args,
                                              const QueryMemoryDescriptor& query_mem_desc,
                                              const ExecutorDeviceType device_type) {
...
llvm::Value* CiderGroupByAndAggregate::getAdditionalLiteral(const int32_t off) {
  CHECK_LT(off, 0);
...
std::vector<llvm::Value*> CiderGroupByAndAggregate::codegenAggArg(
    const Analyzer::Expr* target_expr,
    const CompilationOptions& co) {
  AUTOMATIC_IR_METADATA(executor_->cgen_state_.get());
  const auto agg_expr = dynamic_cast<const Analyzer::AggExpr*>(target_expr);
...
llvm::Value* CiderGroupByAndAggregate::emitCall(const std::string& fname,
                                           const std::vector<llvm::Value*>& args) {
  AUTOMATIC_IR_METADATA(executor_->cgen_state_.get());
  return executor_->cgen_state_->emitCall(fname, args);
}

void CiderGroupByAndAggregate::checkErrorCode(llvm::Value* retCode) {
  AUTOMATIC_IR_METADATA(executor_->cgen_state_.get());
...
size_t CiderGroupByAndAggregate::shard_count_for_top_groups(
    const RelAlgExecutionUnit& ra_exe_unit) {
  // FIXME catalog removed
