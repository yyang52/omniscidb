...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateScalarRex(
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateAggregateRex(
    const RexAgg* rex,
  ...
  if (takes_arg) {
    ...
    if (!cider::is_agg_supported_for_type(agg_kind, arg_ti)) {
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateLiteral(
    const RexLiteral* rex_literal) {
  auto lit_ti = cider::build_type_info(
      rex_literal->getType(), rex_literal->getScale(), rex_literal->getPrecision());
  auto target_ti = cider::build_type_info(rex_literal->getTargetType(),
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateScalarSubquery(
  ...
  Datum d{0};
  bool is_null_const{false};
  std::tie(d, is_null_const) = cider::datum_from_scalar_tv(scalar_tv, ti);
  return makeExpr<Analyzer::Constant>(ti, is_null_const, d);
}

std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateInput(
    const RexInput* rex_input) const {
  // todo  remove
  return nullptr;
}

std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateUoper(
...
// getInIntegerSetExpr().
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateInOper(
    const RexOperator* rex_operator) const {
  if (just_explain_) {
    throw std::runtime_error("EXPLAIN is not supported with sub-queries");
  }
  ...
  if (row_set->entryCount() > 10000) {
    ...
    if ((ti.is_integer() || (ti.is_string() && ti.get_compression() == kENCODING_DICT)) &&
      ...
      expr = cider::get_in_values_expr(lhs, *row_set);
    }
    if (expr) {
      return expr;
    }
...
// CiderRelAlgTranslator::getInIntegerSetExpr makes sure, by checking the encodings, that
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::getInIntegerSetExpr(
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateOper(
    const RexOperator* rex_operator) const {
  ...
  for (size_t i = 1; i < rex_operator->size(); ++i) {
    ...
    std::tie(rhs, sql_qual) = cider::get_quantified_rhs(rhs_op, *this);
    if (!rhs) {
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateOverlapsOper(
    const RexOperator* rex_operator) const {
  const auto sql_op = rex_operator->getOperator();
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateCase(
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateLike(
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateRegexp(
    const RexFunctionOperator* rex_function) const {
  CHECK(rex_function->size() == 2 || rex_function->size() == 3);
  const auto arg = translateScalarRex(rex_function->getOperand(0));
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateLikely(
    const RexFunctionOperator* rex_function) const {
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateUnlikely(
    const RexFunctionOperator* rex_function) const {
  CHECK(rex_function->size() == 1);
  const auto arg = translateScalarRex(rex_function->getOperand(0));
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateExtract(
    const RexFunctionOperator* rex_function) const {
  ...
  cider::validate_datetime_datepart_argument(timeunit_lit);
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateDateadd(
    const RexFunctionOperator* rex_function) const {
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateDatePlusMinus(
    const RexOperator* rex_operator) const {
  ...
  if (rhs_ti.get_type() == kTIMESTAMP || rhs_ti.get_type() == kDATE) {
    ...
    if (rex_operator_ti.get_type() == kINTERVAL_DAY_TIME) {
      return makeExpr<Analyzer::BinOper>(bigint_ti.get_type(),
                                         ...
                                         kONE,
                                         result,
                                         cider::makeNumericConstant(bigint_ti, 1000));
    } else {
      return result;
    }
  }
  ...
  if (op == kPLUS) {
    ...
    auto dt_plus = makeExpr<Analyzer::FunctionOper>(
        datetime_ti, cider::get_datetimeplus_rewrite_funcname(op), args);
  ...
  if (interval_ti.get_type() == kINTERVAL_DAY_TIME) {
    ...
    if (interval_lit) {
      interval_sec = cider::makeNumericConstant(
          bigint_ti,
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateDatediff(
    const RexFunctionOperator* rex_function) const {
  CHECK_EQ(size_t(3), rex_function->size());
  const auto timeunit = translateScalarRex(rex_function->getOperand(0));
  const auto timeunit_lit = std::dynamic_pointer_cast<Analyzer::Constant>(timeunit);
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateDatepart(
    const RexFunctionOperator* rex_function) const {
  CHECK_EQ(size_t(2), rex_function->size());
  const auto timeunit = translateScalarRex(rex_function->getOperand(0));
  const auto timeunit_lit = std::dynamic_pointer_cast<Analyzer::Constant>(timeunit);
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateLength(
    const RexFunctionOperator* rex_function) const {
  CHECK_EQ(size_t(1), rex_function->size());
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateKeyForString(
    const RexFunctionOperator* rex_function) const {
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateSampleRatio(
    const RexFunctionOperator* rex_function) const {
  CHECK_EQ(size_t(1), rex_function->size());
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateCurrentUser(
    const RexFunctionOperator* rex_function) const {
  std::string user{"SESSIONLESS_USER"};
  return Parser::UserLiteral::get(user);
}

std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateLower(
    const RexFunctionOperator* rex_function) const {
  const auto& args = translateFunctionArgs(rex_function);
  CHECK_EQ(size_t(1), args.size());
  CHECK(args[0]);
...
Analyzer::ExpressionPtr CiderRelAlgTranslator::translateCardinality(
    const RexFunctionOperator* rex_function) const {
  ...
  if (array_size > 0) {
    ...
    return cider::makeNumericConstant(ret_ti, array_size / array_elem_size);
  }
  // Variable length array cardinality will be calculated at runtime
  return makeExpr<Analyzer::CardinalityExpr>(arg);
}

std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateItem(
    const RexFunctionOperator* rex_function) const {
  CHECK_EQ(size_t(2), rex_function->size());
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateCurrentDate() const {
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateCurrentTime() const {
  constexpr bool is_null = false;
  Datum datum;
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateCurrentTimestamp() const {
  return Parser::TimestampLiteral::get(now_);
}

std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateDatetime(
    const RexFunctionOperator* rex_function) const {
  CHECK_EQ(size_t(1), rex_function->size());
  const auto arg = translateScalarRex(rex_function->getOperand(0));
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateAbs(
    const RexFunctionOperator* rex_function) const {
  std::list<std::pair<std::shared_ptr<Analyzer::Expr>, std::shared_ptr<Analyzer::Expr>>>
      expr_list;
  CHECK_EQ(size_t(1), rex_function->size());
  ...
  const auto zero = cider::makeNumericConstant(operand_ti, 0);
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateSign(
    const RexFunctionOperator* rex_function) const {
  std::list<std::pair<std::shared_ptr<Analyzer::Expr>, std::shared_ptr<Analyzer::Expr>>>
      expr_list;
  CHECK_EQ(size_t(1), rex_function->size());
  ...
  const auto zero = cider::makeNumericConstant(operand_ti, 0);
  const auto lt_zero = makeExpr<Analyzer::BinOper>(kBOOLEAN, kLT, kONE, operand, zero);
  expr_list.emplace_back(lt_zero, cider::makeNumericConstant(operand_ti, -1));
  const auto eq_zero = makeExpr<Analyzer::BinOper>(kBOOLEAN, kEQ, kONE, operand, zero);
  expr_list.emplace_back(eq_zero, cider::makeNumericConstant(operand_ti, 0));
  const auto gt_zero = makeExpr<Analyzer::BinOper>(kBOOLEAN, kGT, kONE, operand, zero);
  expr_list.emplace_back(gt_zero, cider::makeNumericConstant(operand_ti, 1));
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateOffsetInFragment() const {
  return makeExpr<Analyzer::OffsetInFragment>();
}

Analyzer::ExpressionPtr CiderRelAlgTranslator::translateArrayFunction(
    const RexFunctionOperator* rex_function) const {
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateFunction(
    const RexFunctionOperator* rex_function) const {
  ...
  try {
    ...
    LOG(WARNING) << "CiderRelAlgTranslator::translateFunction: " << e.what();
    throw;
  }
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateWindowFunction(
    const RexWindowFunctionOperator* rex_window_function) const {
  if (!cider::supported_lower_bound(rex_window_function->getLowerBound()) ||
      !cider::supported_upper_bound(rex_window_function) ||
  ...
  return makeExpr<Analyzer::WindowFunction>(
      ...
      partition_keys,
      order_keys,
      cider::translate_collation(rex_window_function->getCollation()));
}

Analyzer::ExpressionPtrVector CiderRelAlgTranslator::translateFunctionArgs(
    const RexFunctionOperator* rex_function) const {
  std::vector<std::shared_ptr<Analyzer::Expr>> args;
...
std::shared_ptr<Analyzer::Expr> CiderRelAlgTranslator::translateHPTLiteral(
    const RexFunctionOperator* rex_function) const {
